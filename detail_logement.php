...
<head>
    ...
    <style>
        ...
        .carousel-control-next-icon:after {
            content: '❮'; /* Custom content for 'previous' */
            font-size: 24px; /* Adjust size as needed */
            color: black; /* Arrow color */
        }
        .carousel-control-next-icon:after {
            content: '❯'; /* Custom content for 'next' */
        }
        .carousel-img {
...
<body>
    <?php
    session_start();
    require_once 'include/connection_db.php'; // Connection to the database
    ...
    if ($logementId) {
        ...
        // Convertir les dates en format JSON pour utilisation dans JavaScript
    ...
    for (let i = 0; i < 6; i++) {
        ...
        for (let j = 1; j <= 7; j++) {
            if (i === 0 && j < premierJourSemaine || jourCourant > nombreDeJours) {
                ...
                let backgroundColor = isReserved ? '#696969' : isSelected ? '#FFA500' : isAvailable ? '#28a745' : '#FFFFFF'; // Gris foncé pour réservé, orange pour sélectionné, vert pour disponible, blanc pour non-disponible
...
let prixParNuit = <?= json_encode($logement['prix']) ?>; // Récupérer le prix par nuit depuis PHP
